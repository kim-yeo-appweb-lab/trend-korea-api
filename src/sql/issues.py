...
class IssueRepository:
    ...
    def list_triggers(
        ...
        *,
        ...
        stmt = (
            select(Trigger).where(Trigger.issue_id == issue_id).order_by(desc(Trigger.occurred_at))
        )
        rows = self.db.execute(stmt.offset(offset).limit(size + 1)).scalars().all()
        has_next = len(rows) > size
        items = rows[:size]
        ...
        if related_event_ids is not None:
            ...
            if related_event_ids:
                self.db.execute(
                    insert(issue_events),
                    [
                        {"issue_id": issue.id, "event_id": event_id}
    ...
    def list_tracked_at_by_issue_ids_for_user(
        self, *, user_id: str, issue_ids: list[str]
    ) -> dict[str, datetime]:
        if not issue_ids:
            return {}
