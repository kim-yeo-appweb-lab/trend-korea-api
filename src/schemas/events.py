...
class EventListQuery(BaseModel):
    ...
    importance: str | None = Field(
        default=None,
        alias="filter[importance]",
        description="중요도 필터 (low, medium, high)",
        examples=["high"],
    )
    verification_status: str | None = Field(
        default=None,
        alias="filter[verificationStatus]",
        description="검증 상태 필터 (verified, unverified)",
        examples=["verified"],
    )
