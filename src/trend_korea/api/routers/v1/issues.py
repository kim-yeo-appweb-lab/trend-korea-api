...
from trend_korea.api.schemas.issues import CreateIssueRequest, CreateTriggerRequest, UpdateIssueRequest
...
def list_issues(
    ...
    status: str | None = Query(default=None, pattern="^(ongoing|closed|reignited|unverified)$", description="이슈 상태 필터 (ongoing, closed, reignited, unverified)"),
    ...
    sortBy: str = Query(default="updatedAt", description="정렬 기준 필드 (updatedAt, latestTriggerAt, createdAt)"),
...
def create_issue(payload: CreateIssueRequest, request: Request, db: DbSession, _: CurrentAdminUserId):
    service = IssueService(IssueRepository(db))
...
def update_issue(issue_id: str, payload: UpdateIssueRequest, request: Request, db: DbSession, _: CurrentAdminUserId):
    service = IssueService(IssueRepository(db))
